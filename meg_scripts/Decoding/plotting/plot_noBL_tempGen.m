sq_ps ='D:\bbarnett\Documents\ecobrain\data\results\group\stats\one_sample\Decoding\Cross\Temporal\multiclass\pVals_multiclass_PAS_trainSquares_noBL_cv';
di_ps = 'D:\bbarnett\Documents\ecobrain\data\results\group\stats\one_sample\Decoding\Cross\Temporal\multiclass\pVals_multiclass_PAS_trainDiamonds_noBL_cv';
cfg = [];
cfg.diFile = 'multiclass_PAS_trainDiamonds_noBL_cv';
cfg.sqFile = 'multiclass_PAS_trainSquares_noBL_cv';
...
remove=false;
di_acc = zeros(length(subjects),length(time(1:614)),length(time(1:614)));
sq_acc=zeros(length(subjects),length(time(1:614)),length(time(1:614)));
for subj =1:length(subjects)
    ...
    %di_acc(subj,:,:) = diam_acc(1:614,1:614);
    %sq_acc(subj,:,:) = square_acc(1:614,1:614);
    di_acc(subj,:,:) = train_di_cross_acc(1:614,1:614);
    sq_acc(subj,:,:) = train_sq_cross_acc(1:614,1:614);
...
%% Plot
 %Create Mask of Significant P Values
    ...
    sq_mask = pVals_onesamp_sq(1:614, 1:614);
    colormap jet;
   ...
   load(strcat(di_ps,'.mat'))
    di_mask = pVals_onesamp_di(1:614, 1:614);
    colormap jet;
