function clusterPvals = cluster_based_permutationND(data1,data2_or_chance_level,cfg,mask,connectivity)
...
%
% Edited by Nadine Dijkstra August 2017
%
...
%print lowest non-significant value for reporting
ps = [unique(pPos) unique(pNeg)];
ps = ps(ps>0.05);
fprintf('The non-significant p-values are over %.2f \n',min(ps))
