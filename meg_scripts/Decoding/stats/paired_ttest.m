function [t,p,h] = paired_ttest(cfg, subjects)
...
%if ~exist(fullfile(outputDir,[cfg.outputName '.mat']),'file')
    ...
    for subj =1:length(subjects)
        ...
        load(strcat('../data/results/',subject','/',cfg.crossDir,'/',cfg.accFile), 'Accuracy');
        acc = Accuracy;
        all_acc1(subj,:,:) = acc;
        
        clear acc Accuracy
        
        if contains(cfg.crossDir,'Square')
            acc = load(strcat('../data/results/',subject','/',cfg.withinDir,'/',[cfg.accFile,'_square']), 'Accuracy');
        elseif contains(cfg.crossDir,'Diam')
            acc = load(strcat('../data/results/',subject','/',cfg.withinDir,'/',[cfg.accFile,'_diamond']), 'Accuracy');
        end
        
        all_acc2(subj,:,:) = acc.Accuracy;
        clear acc Accuracy 
