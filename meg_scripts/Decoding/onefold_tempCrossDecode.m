...
%if ~exist(fullfile(outputDir,strcat(cfg.outputName, '.mat')),'file')
    ...
    Xhat = zeros(nTestSamples,nTestSamples,nTestTrials);
    
    
    trainidx = 1:nTrainTrials; %all trials as this is cross classification
    testidx = 1:nTestTrials; 
    ...
    Accuracy = squeeze(mean((Xhat>0)==permute(repmat(Y_test,1,nTestSamples,nTestSamples),[2,3,1]),3));
