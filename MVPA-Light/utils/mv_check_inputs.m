...
if n_classes > 2 && ismember(cfg.classifier, binary_classifiers)
    if any(contains(cfg.preprocess,'undersample_multibin'))
        warning('Using a binary %s decoder for %d class labels. Make sure you are coding a binary decoder via undersampling function.', upper(cfg.classifier), n_classes)
    else
