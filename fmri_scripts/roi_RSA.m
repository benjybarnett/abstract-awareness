function  pval =  roi_RSA(cfg)
    ...
    for subj = 1:length(cfg.subjects)
        ...
        save(fullfile(cfg.output_dir,cfg.subjects{subj},cfg.roi,'true_rho.mat'),'rho');
    ...
    for subj = 1:length(cfg.subjects)
        ...
        true_rho = load(fullfile(cfg.output_dir,cfg.subjects{subj},cfg.roi,'true_rho.mat'));
        emp_dist = [emp_dist true_rho.rho];
    end
    
    
    pval = sum(rho_btstrp > mean(emp_dist))/cfg.nBtrsp;
   
    fprintf('\t RSA within the %s ROI has a p value  of %d \n',cfg.roi,pval);
    
    %save the null distribution
    save(fullfile(cfg.output_dir,'Group',cfg.roi,cfg.modelRDM(1:end-4),'Group_Null_Distribution.mat'),'perm_rhos')
