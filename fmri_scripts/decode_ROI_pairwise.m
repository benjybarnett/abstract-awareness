function [pval] = decode_ROI_pairwise(cfg)

%function to cross decode using a multivoxel pattern taken from an ROI,
%decoding only two stim classes at a time
...
for subj = 1:length(cfg.subjects)
    ...
    for trl = 1:length(trials)
        ...
        if strcmp(trial(12), cfg.pairs{1})
            
            Y = [Y 0]; %0 for class 1
        elseif strcmp( trial(12), cfg.pairs{2})
            Y = [Y 1]; %1 for class 1
        end  
    end
    ...
    save(fullfile(outputDir,[cfg.pair_name,'_acc.mat']),'acc')
    ...
    % save
    save(fullfile(outputDir,strcat(cfg.pair_name,'_accuracyPerm.mat')),'accuracy')
end
    ...
    for subj = 1:length(cfg.subjects)
        ...
        if exist(fullfile(outputDir,strcat(cfg.pair_name,'_accuracyPerm.mat')),'file')

            fprintf('Loading subj %d \n',subj)
            load(fullfile(outputDir,strcat(cfg.pair_name,'_accuracyPerm.mat')),'accuracy')
    ...
    for subj = 1:length(cfg.subjects)
        ...
        true_acc = load(fullfile(cfg.output_dir,cfg.subjects{subj},cfg.roi,strcat(cfg.pair_name,'_acc.mat')));
